    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-autocomplete
          ...
          item-value="id" label="Pflanzentyp suchen"
          variant="outlined"
          density="compact"
          clearable
          chips
          closable-chips
          return-object :loading="isSearchingPlantTypes"
...
const mockSearchPlantTypes = async (query: string): Promise<PlantType[]> => {
  ...
  const allMockPlantTypes: PlantType[] = [
    { id: '1', latName: 'Solanum lycopersicum', commonName: 'Tomate', description: 'Leckere rote Frucht.' },
    { id: '2', latName: 'Capsicum annuum', commonName: 'Paprika', description: 'Vielseitiges Gemüse.' },
    { id: '3', latName: 'Mentha spicata', commonName: 'Minze', description: 'Frisches Kraut für Tee.' },
    { id: '4', latName: 'Fragaria x ananassa', commonName: 'Erdbeere', description: 'Beliebte Sommerfrucht.' },
    { id: '5', latName: 'Ocimum basilicum', commonName: 'Basilikum', description: 'Aromatische Kräuter.' },
  ];

  // Filtere basierend auf der Abfrage (case-insensitive und startsWith/includes)
  const lowerCaseQuery = query.toLowerCase();
  return allMockPlantTypes.filter(type =>
...
const createPlant = () => {
  ...
  const newPlantData = {
    ...
    plantTypeId: selectedPlantType.value?.id || selectedPlantType.value?.latName, // Verwende ID oder latName als Referenz

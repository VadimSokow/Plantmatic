...
import src.helper.toml as toml
from src.Mqtt.DeviceClient import DeviceClient
from src.PlantManagement.Plant import Plant
from src.PlantManagement.PlantManagerInterface import PlantManagerInterface
from src.logger import create_logger

logger = create_logger(__name__)


def load_plants(plant_config_path: str) -> list[Plant]:
    ...
    for name, config in plant_config.items():
        ...
        plant_object = Plant(p_name, p_measuring_interval, p_min_humidity, p_max_humidity, p_slot_num)
...
class PlantManager(PlantManagerInterface):
    ...
    async def start(self, device_client: DeviceClient, shutdown_event: Event) -> None:
        ...
        self.plants = load_plants(self.plant_config_path)
    ...
    def push_new_plant_config(self, new_config):
        ...
        self.new_plants = load_plants(self.plant_config_path)
        ...
        for name, config in patch.items():
            ...
            if name != "$version":
                ...
                for key, value in config.items():
                    logger.debug(f"Plant: {name}, with key: {key}, and value: {value}")
